...
public class InventoryTestActivity extends Activity {
            ...
            new AsyncTask<Void, Void, Item>() {
                ...
                protected Item doInBackground(Void... params) {
                    ...
                    try {
                        ...
                        List<Item> items = inventoryService.getItemsWithCategories(getItemsStatus);
    ...
    private String dumpItem(Item item) {
        return item != null ? String.format(Locale.US, "%s{id=%s, name=%s, price=%d}", Item.class.getSimpleName(), item.getId(), item.getName(), item.getPrice()) : null;
    }

    private String dumpTaxRate(TaxRate taxRate) {
        return String.format(Locale.US, "%s{id=%s, name=%s, rate=%d, is default=%b}", TaxRate.class.getSimpleName(), taxRate.getId(), taxRate.getName(), taxRate.getRate(), taxRate.getIsDefault());
    }

    private String dumpModifier(Modifier modifier) {
        return String.format(Locale.US, "%s{id=%s, name=%s}", Modifier.class.getSimpleName(), modifier.getId(), modifier.getName());
    }

    private String dumpModifierGroup(ModifierGroup modifierGroup) {
        return String.format(Locale.US, "%s{id=%s, name=%s}", ModifierGroup.class.getSimpleName(), modifierGroup.getId(), modifierGroup.getName());
    }
    ...
    static class InventoryWebService implements IInventoryService, com.clover.android.sdk.examples.InventoryWebService {
